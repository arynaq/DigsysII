...
class PID{
  ...
  */
  ...
  PID(float* output, float* newPoint, float* setPoint,
         int dt, float KP, float KI, float KD){
    this->output = output;
    this->newPoint = newPoint;
    this->setPoint = setPoint;
    this->dt = dt;
    this->KP = KP;
    this->KI = KI*dt/1000.0;
    this->KD = KD/(dt/1000.0);
  }
  ...
  void setGains(float _KP, float _KD, float _KI){
    setKP(_KP);
    setKD(_KD);
    setKI(_KI);
  }
...
private:
  ...
  */
  float sum = 0;
  /*
  The sampletime, in milliseconds, usually 100, time between outputs from PID
  */
  int dt;
  /*
